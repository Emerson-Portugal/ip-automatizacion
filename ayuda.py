...
def clase_red(ip, subRed_nueva, subRed):
    ...
    if 0 <= int(separado[0]) <= 127:
        mascara = "255.0.0.0"
        valor_clase = "A"
        print(f"\n Clase {valor_clase} \n")
        direccion_red(ip, mascara, subRed_nueva, valor_clase, subRed)
    elif 128 <= int(separado[0]) <= 191:
        mascara = "255.255.0.0"
        valor_clase = "B"
        print(f"\n Clase {valor_clase} \n")
        direccion_red(ip, mascara, subRed_nueva, valor_clase, subRed)
    elif 192 <= int(separado[0]) <= 223:
...
def direccion_red(ip, mascara, subRed_nueva, valor_clase, subRed):
    ...
    obtener_nueva_mascara(direcionRed, separado_mascara, subRed_nueva, valor_clase, subRed)


def obtener_nueva_mascara(direcionRed, mascara, subRed_nueva, valor_clase, subRed):
...
def tabla_red(direcionRed, salto_red_nueva, host_nuevo, subRed, valor_clase_nueva):
    ...
    for i in range(subRed):
        ...
        print("{:<19s} {:<15s} {:<23s} {:<23s} {:<15s}".format(subRed_nueva, str(direccion_ip_suma_p),
                                                               str(direccion_ip_suma_u), str(direccion_ip_suma_b),
                                                               str(host_nuevo)))

        salto_red_nueva_valor += salto_red_nueva
